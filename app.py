...
# --- PAGE INITIALIZATION ---
st.set_page_config(
    page_title="AlgoMetrics Pro Terminal",
    page_icon="⚡",
    layout="wide",
...
with app_mode[0]:
    ...
    if uploaded_screenshot:
        if ticker_input:
            with st.status("Syncing Terminal...", expanded=False):
            ...
            if error: st.error(f"Error: {error}")
                ...
                with left_col:
                    ...
                    # NATIVE JAVASCRIPT CHART RUNNING DIRECTLY INSIDE PYTHON 3.14
                            ...
                            new Chart(ctx, {{
                                ...
                                data: {{
                                    ...
                                    datasets: [
                                        ...
                                        {{
                                            label: 'EMA 20 Structural Baseline',
                                            data: {ema20_values},
                                            borderColor: '#2563eb',
                                            borderWidth: 1.5,
                                            borderDash: [5, 5],
                    ...
                    st.markdown("### 🏢 Fundamental Structure Context")
                    with st.container(border=True):
                        f_c1, f_c2 = st.columns(2)
                        m_cap_g = data["marketCap"] / 1e9
                        shares_out_m = data["sharesOutstanding"] / 1e6
                        f_c1.metric("Market Cap ($ Billions)", f"${m_cap_g:,.2f}B" if m_cap_g > 0 else "N/A")
                        f_c2.metric("Share Float (Millions)", f"{shares_out_m:,.1f}M" if shares_out_m > 0 else "N/A")
                ...
                with right_col:
                    ...
                    sl = entry - (data["atr"] * 1.5) if data["trend"] == "BULLISH" else entry + (data["atr"] * 1.5)
                    ...
                    total_capital_deployed = (shares * entry) + total_fees
                    ...
                    with t1:
                        with st.container(border=True):
                            ...
                            st.divider()
                            st.write(f"📦 **Volume:** `{shares}` Units | 💵 **Fees:** `${total_fees:,.2f}` | 📊 **R:R:** `1 : {rr:.2f}`")
                    with t2:
                        with st.container(border=True):
...
# ==============================================================================
# TAB 2: MACRO MARKET RADAR WITH NATIVE JAVASCRIPT SPARKLINES
# ==============================================================================
with app_mode[1]:
    ...
    def fetch_macro_radar_matrix():
        ...
        for market_name, symbol in major_markets.items():
            try:
                ...
                # Pull the raw historical close timeline data for JS graphing
    ...
    if macro_matrix:
        ...
        for i, (m_name, m_data) in enumerate(macro_matrix.items()):
            with grid_cols[i]:
                with st.container(border=True):
                    ...
                    # NATIVE JAVASCRIPT MICRO SPARKLINES FOR RADAR CARDS
        ...
        with st.container(border=True):
            st.markdown("""
            **How Day Traders Apply the Macro Market Radar:**
            * **Market Directional Bias:** When 3 out of 4 indexes display `🟩 BUY LIMIT ALIGNED`, the global market possesses institutional buying pressure. Avoid looking for short setups on individual stocks.
